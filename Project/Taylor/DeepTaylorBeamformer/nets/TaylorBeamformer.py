...
from Project.Taylor.DeepTaylorBeamformer.torch_complex.tensor import ComplexTensor
import Project.Taylor.DeepTaylorBeamformer.torch_complex.functional as F
from Project.Taylor.DeepTaylorBeamformer.utils.utils import complex_mul, complex_conj, NormSwitch
import math
...
class TaylorBeamformer(nn.Module):
            ...
            if is_param_share:
                ...
                for i in range(order_num):
                    highorderblock_list.append(
                        HighOrderBlock(kd1, cd1, d_feat, dilations, group_num, fft_num, is_1dgate,
                                       is_causal, is_squeezed, norm1d_type))
    ...
    def forward(self, inpt):
        ...
        if self.is_compress:
            ...
            spatial_x = torch.stack((spatial_mag * torch.cos(spatial_phase), spatial_mag * torch.sin(spatial_phase)),
                                    dim=1)
        else:
...
class ZeroOrderBlock(nn.Module):
    ...
    def forward(self, inpt):
        ...
        for i in range(self.group_num):
            ...
            x_acc += x
        x = x_acc
        x = x.view(b_size, -1, 4, seq_len).transpose(-2,
                                                     -1).contiguous()  # 4 denotes the freq size of the last encoding layer
...
class BeamformingModule(nn.Module):
    ...
    def forward(self, inpt1, inpt2=None):
        ...
        if self.bf_type == "mvdr":
            ...
            inverse_phi = inverse_phi.view(b_size, freq_num, seq_len, self.M, self.M, 2).transpose(1,
                                                                                                   2)  # (B,T,F,M,M,2)
            # mvdr
...
if __name__ == "__main__":
    ...
    from Project.Taylor.DeepTaylorBeamformer.utils.utils import numParams
